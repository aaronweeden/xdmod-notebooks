...
## Install/upgrade the `xdmod-data` package

```{r}
py_install("xdmod-data>=1.0.0,<2.0.0", pip = TRUE)
```

## Define the XDMoD colors and Plotly layout
...
plot <- ggplot(
  ...
  geom_line(color = xdmod_colors[1]) +
  geom_point(color = xdmod_colors[1])
...
plot <- ggplot(
  ...
  geom_boxplot(color = xdmod_colors[1])
...
df <- data %>%
  ...
  pivot_longer(
    -Time,
    names_to = dimension_label,
    values_to = metric_label
  )
...
plot <- ggplot(
  ...
  aes(
    x = `Time`,
    y = .data[[metric_label]],
    color = .data[[dimension_label]]
  )
) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = xdmod_colors)
...
The **dataset type** can either be 'timeseries' (the default), in which data is grouped by a time [aggregation unit](#Aggregation-unit), or 'aggregate', in which the data is aggregated across the entire. For 'aggregate', the results are returned as a Pandas Series rather than a DataFrame.

The code chunks below show how to create a bar plot of data aggregated over four months, grouped by resource. Note that using Python to convert the Series to a DataFrame is a workaround for an error in Reticulate.
